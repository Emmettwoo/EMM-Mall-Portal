...
let cities = {
    ...
    getCities: function (provinceId) {
        // console.log("cities获取可选列表：城市(provinceId" + provinceId + ")");
        let cities = [];
        for (let i=0, length=data[provinceId]["city"].length; i<length; i++) {
            cities.push(data[provinceId]["city"][i].name);
        }
        return cities;
    },
    getDistricts: function (provinceId, cityId) {
        // console.log("cities获取可选列表：区县(provinceId" + provinceId + ", cityId" + cityId + ")");
        let districts = [];
        for (let i=0, length=data[provinceId]["city"][cityId].districtAndCounty.length; i<length; i++) {
            districts.push(data[provinceId]["city"][cityId].districtAndCounty[i]);
        }
        return districts;
    },
    ...
    getProvince: function (provinceId) {
        // console.log("cities获取目标名称：省份(provinceId" + provinceId + ")");
        return data[provinceId].name;
    },
    getCity: function (provinceId, cityId) {
        // console.log("cities获取目标名称：城市(provinceId" + provinceId + ", cityId" + cityId + ")");
        return data[provinceId]["city"][cityId].name;
    },
    getDistrict: function (provinceId, cityId, districtId) {
        // console.log("cities获取目标名称：区县(provinceId" + provinceId + ", cityId" + cityId + ", districtId" + districtId + ")");
        return data[provinceId]["city"][cityId].districtAndCounty[districtId];
    },

    // 根据字段获取Id
    getProvinceId: function (provinceName) {
        // console.log("cities获取目标编号：省份(provinceName" + provinceName + ")");
        for (let i=0, length=data.length; i<length; i++) {
            if (provinceName === data[i]['name']) {
                return i;
            }
        }
        return 0;
    },
    getCityId: function (provinceId, cityName) {
        // console.log("cities获取目标编号：城市(provinceId" + provinceId + ", cityName" + cityName + ")");
        for (let i=0, length=data[provinceId]["city"].length; i<length; i++) {
            if (cityName === data[provinceId]['city'][i].name) {
                return i;
            }
        }
        return 0;
    },
    getDistrictId: function (provinceId, cityId, districtName) {
        // console.log("cities获取目标编号：区县(provinceId" + provinceId + ", cityId" + cityId + ", districtName" + districtName + ")");
        for (let i=0, length=data[provinceId]["city"][cityId].districtAndCounty.length; i<length; i++) {
            if (districtName === data[provinceId]['city'][cityId].districtAndCounty[i]) {
                return i;
            }
        }
        return 0;
